...
var ediCarrierCode = norm(carrier.carrierCode)
var ediRouting     = norm(carrier.routing)
var p21CarrierCode = norm(p21Items[0].carrier_code)

var carrierErrors = flatten([

    if (isEmpty(ediCarrierCode) and isEmpty(ediRouting))
        ["Missing Carrier"]
    else [],

    if (!isEmpty(ediCarrierCode) and sizeOf(ediCarrierCode) < 2)
        ["Invalid SCAC"]
    else [],

    if (!isEmpty(p21CarrierCode) and !isEmpty(ediCarrierCode) and ediCarrierCode != p21CarrierCode)
        ["Carrier mismatch"]
    else []
])
