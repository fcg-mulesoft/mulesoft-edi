...
var shipToErrors =
    flatten([
        if (isMismatch(isMatch(shipTo.name, firstMatched.ship2_name)))
            ["ShipTo Name mismatch"]
        else
            [],
        if (addressMatchPercentage(shipTo.address1, firstMatched.ship2_add1) <= ((Mule::p('shipToValidation.lowerLimit')) as Number))
            ["ShipTo Address1 mismatch"]
        else
            [],
        if (isMismatch(isMatch(shipTo.city, firstMatched.ship2_city)))
        ...
        if (DEBUG)
            {
                ...
                shipToMatchPercentage: addressMatchPercentage(shipTo.address1, firstMatched.ship2_add1)
            }
        else
            null,
