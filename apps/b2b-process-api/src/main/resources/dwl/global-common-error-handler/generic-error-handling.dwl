...
var msgVendorId = Mule::p('partner.' ++ (safe(cdmHeader.senderId, safe(integration.source, "N/A")) as String))
...
var errorResolution =
    if (errorCategory == "VALIDATION")
        if (isValidationFlow)
            ...
            ++ "\n  1. This IS a data issue — review the payload sent from " ++ sourceSystem ++ " for incorrect values, wrong data types, or fields that fall outside allowed values."
            ++ "\n  2. Compare the submitted data against the API schema or contract to identify non-conforming fields."
            ...
            ++ "\n  2. Ensure the source system " ++ sourceSystem ++ " is populating all mandatory fields before sending."
            ...
            ++ "\n  1. This is a data formatting issue — the file or message may be corrupted, incomplete, or in the wrong format."
            ++ "\n  2. Verify that the Content-Type of the message matches the actual format (e.g. JSON, XML)."
            ++ "\n  3. Check for special characters, encoding problems, or truncated content in the payload."
        ...
        do {
            ...
            var steps =
                if (categoryId == "TIMEOUT" or categoryId == "CONNECTION_TIMEOUT" or categoryId == "GATEWAY_TIMEOUT")
                    ...
                    "\n  1. The connection to " ++ targetSystem ++ " was made but the response took too long."
                    ++ "\n  2. This may be caused by a large payload or a slow response from " ++ targetSystem ++ "."
                    ++ "\n  3. Contact the integration support team with the Correlation ID to investigate."
        ...
        do {
            ...
            var steps =
                if (categoryId == "UNAUTHORIZED" or categoryId == "AUTHENTICATION_FAILED")
                    ...
                    "\n  1. The system connected successfully but does not have permission to perform this action in " ++ targetSystem ++ "."
                    ++ "\n  2. This is a permissions or role configuration issue — contact the IT support team."
                    ++ "\n  3. Do not retry — the transaction will continue to fail until permissions are corrected."
                else if (categoryId == "TOKEN_EXPIRED" or categoryId == "INVALID_TOKEN")
                    "\n  1. The security token used to connect to " ++ targetSystem ++ " has expired or is no longer valid."
            ...
            ---
            base ++ steps
            ++ "\n\n  Do not retry the transaction until the access issue has been resolved by the support team."
        }
        ...
        do {
            ...
            var steps =
                if (categoryId == "NOT_FOUND")
                    "\n  1. The record or resource being referenced does not exist in " ++ targetSystem ++ "."
                    ++ "\n  2. Verify the PO Number, vendor ID, or other reference data is correct."
                    ++ "\n  3. Check whether the record was recently deleted, archived, or moved in " ++ targetSystem ++ "."
                    ...
                    ++ "\n  2. Check whether the transaction has already been processed — look up the PO Number in " ++ targetSystem ++ "."
                    ++ "\n  3. If the record already exists and is correct, no further action may be needed."
                    ++ "\n  4. Contact the support team with the Correlation ID if you are unsure."
                else if (categoryId == "TOO_MANY_REQUESTS")
                    "\n  1. Too many requests were sent to " ++ targetSystem ++ " in a short period of time."
                    ++ "\n  2. Wait a few minutes before resubmitting the transaction."
                    ++ "\n  3. Contact the integration support team if this issue occurs frequently."
                else
        ...
        do {
            ...
            var steps =
                if (categoryId == "COERCION")
                    "\n  1. A field value could not be converted to the expected data type (e.g. a text value was found where a number was expected)."
...
var data = {
    ...
    partnerName:     if (isValidationFlow) Mule::p('partner.' ++ safe(cdmHeader.senderId))
                     else if (msgVendorId != "N/A") msgVendorId
                     else sourceSystem,
