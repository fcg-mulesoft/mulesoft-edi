...
var ADDRESS_MATCH_TOLERANCE = 50
...
fun isAddressMatch(a,b)=
  isEmpty(b default "") or addressMatchPercentage(a,b)>=ADDRESS_MATCH_TOLERANCE
...
var comparison = items map (line) -> do {
    ...
    var matched =
      (if (!isEmpty(norm(buyerPart default ""))) p21Index[norm(buyerPart)] else null)
      default (if (!isEmpty(norm(vendorPart default ""))) p21Index[norm(vendorPart)] else null)
      default {}
    ...
    ---
    {
      ...
      supplier_part_no: {
        ...
        match:
          isEmpty(vendorPart default "") or
          isEmpty(matched.supplier_part_no default "") or
          isMatch(vendorPart, matched.supplier_part_no)
      },
      qty_ordered: {
      ...
      ship2_add1: {
        ...
        match: isAddressMatch(shipTo.address1, p21Ship.ship2_add1),
      ...
      shipping_instruction: {
        ...
        match:
          isEmpty(p21Ship.shipping_instruction) or
      ...
      carrier_id: {
        ...
        odata: norm(p21Ship."carrier_code"),
        match:
          isEmpty(norm(summary.carrierDetail.carrierCode)) or
          isEmpty(norm(p21Ship."carrier_code")) or
          isMatch(summary.carrierDetail.carrierCode, p21Ship."carrier_code")
      },
      external_po_no: {
        original: header.poNumber,
        odata: p21Ship."po_no",
        match:
          sizeOf(p21Items filter ($.po_type == "D")) == 0 or
          isEmpty(p21Ship."po_no") or
          isMatch(header.poNumber, p21Ship."po_no")
      }
    }
}
